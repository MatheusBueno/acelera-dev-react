...
describe("Users actions", () => {
  it("Should create an action to fetch users", () => {
    ...
    expect(usersActions.fetchUsers({ query: "anderson" })).toEqual(
      expectedAction
    );
  });
  ...
  it("Should create an action to fetch users with success", () => {
    ...
    expect(usersActions.fetchUsersSuccess({ items: [] })).toEqual(
      expectedAction
    );
  });
  ...
  it("Should create an action to fetch users with failure", () => {
    ...
    expect(usersActions.fetchUsersFailure({ message: "Error" })).toEqual(
      expectedAction
    );
  });
...
describe("Users reducer", () => {
  ...
  it("Should handle FETCH_USERS_SUCCESS", () => {
    const action = {
      type: usersTypes.FETCH_USERS_SUCCESS,
      payload: { items: [...usersMock.items] }
    };
    const expectedState = {
      ...state,
      isLoading: false,
      pages: 5,
      users: [...usersMock.items]
    };
    expect(reducer(state, action)).toEqual(expectedState);
  });
  ...
  it("Should handle FETCH_USERS_FAILURE", () => {
    const action = {
      type: usersTypes.FETCH_USERS_FAILURE,
      payload: { message: "Error on fetch users." }
    };
    const expectedState = {
      ...state,
      isLoading: false,
  ...
  it("Should handle SELECT_USER", () => {
    const action = {
      type: usersTypes.SELECT_USER,
      payload: { user: usersMock.items[0] }
    };
